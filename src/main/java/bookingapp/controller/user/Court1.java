...
public class Court1 {
    ...
    private void initialize(){
        ...
        Total = 0;
        total.setVisible(false);
        lb_header.setText("Đặt Lịch Sân 1");
        ...
        fld_date.setOnAction(e->{
            fld_date.show();
            picked_date = fld_date.getValue().toString();
            LichNgoaiLe = LoadDAO.loadAllLichNgoaiLe(picked_date, 1);
            LichDat = LoadDAO.loadLichDat(picked_date, 1);
            loadStatus();
        });
        LichNgoaiLe = LoadDAO.loadAllLichNgoaiLe(picked_date, 1);
        LichDat = LoadDAO.loadLichDat(picked_date, 1);
    ...
    private void handleConfirm(){
        ...
        for(int h = 0; h < 24; h++){
            if(picked.containsKey(h)){
                ...
                boolean Add = Bookingdao.add(new Booking(user.getId(), 1, LocalDate.parse(picked_date), LocalTime.parse(start), LocalTime.parse(end), res_total));
                if(Add){
