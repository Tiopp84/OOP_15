...
public class Court3 {
    ...
    private void initialize(){
        ...
        fld_date.setOnAction(e->{
            fld_date.show();
            picked_date = fld_date.getValue().toString();
            LichNgoaiLe = LoadDAO.loadAllLichNgoaiLe(picked_date, 3);
            LichDat = LoadDAO.loadLichDat(picked_date, 3);
            loadStatus();
        });
        LichNgoaiLe = LoadDAO.loadAllLichNgoaiLe(picked_date, 3);
        LichDat = LoadDAO.loadLichDat(picked_date, 3);
    ...
    private void handleConfirm(){
        ...
        for(int h = 0; h < 24; h++){
            if(picked.containsKey(h)){
                ...
                boolean Add = Bookingdao.add(new Booking(user.getId(), 3, LocalDate.parse(picked_date), LocalTime.parse(start), LocalTime.parse(end), res_total));
                if(Add){
